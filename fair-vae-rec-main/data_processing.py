...
def preprocess_movielens(
    ...
    movie_info = pd.read_csv(
        os.path.join(DATA_DIR, movies_filename),
        sep="::",
    ...
    movie_info = movie_info.loc[:, ["item_id"]]

    # Call general preprocessing function
    preprocess(PROCESSED_DATA_DIR, validation_frac, balance_sensitive, user_split, ratings, user_info, movie_info)
...
def preprocess(
    ...
    unique_train_items = movie_info["item_id"].unique()
    ...
    save_df(PROCESSED_DATA_DIR, movie_info, "movie_info.csv", index=False)
...
def load_and_uncompress(PROCESSED_DATA_DIR, user_split=True, csr=False, uncompress=True):
    ...
    with open(os.path.join(PROCESSED_DATA_DIR, "unique_items.txt"), "r") as f:
        for line in f:
            unique_item_ids.append(int(line.strip()))
    n_items = max(unique_item_ids) + 1
