...
from utils import (
    ...
    chi_square_rec_k_gender,
    kendall_tau_rec_gender,
    chi_square_rec_k_age,
    kendall_tau_rec_age,
    get_aggregated_item_ranks_chi_gender,
    get_aggregated_item_ranks_ken_gender,
    get_aggregated_item_ranks_chi_age,
    get_aggregated_item_ranks_ken_age,
)
...
def parse_arguments():
    ...
    parser.add_argument("--data_dir", type=str, default="data/ml-1m", help="PREPROCESSING: Path to data directory")
...
def evaluate_all_recommendations(settings, u_probs, targets, s):
    ...
    chi100.append(chi_square_rec_k_gender(u_probs, s[:, 0], settings.indv_k, settings.n_chi2_items))
    kendall100.append(kendall_tau_rec_gender(u_probs, s[:, 0], settings.indv_k, settings.agg_k))
    chi100.append(chi_square_rec_k_age(u_probs, s[:, 1], settings.indv_k, settings.n_chi2_items))
    kendall100.append(kendall_tau_rec_age(u_probs, s[:, 1], settings.indv_k, settings.agg_k))
...
def evaluate_partial_recommendations(settings, u_probs, targets, s, eval_res):
    ...
    chi_ranks.append(get_aggregated_item_ranks_chi_gender(u_probs, s[:, i], settings.indv_k, False))
    kendall_ranks.append(get_aggregated_item_ranks_ken_gender(u_probs, s[:, i], settings.indv_k, True))
    chi_ranks.append(get_aggregated_item_ranks_chi_age(u_probs, s[:, i], settings.indv_k, False))
    kendall_ranks.append(get_aggregated_item_ranks_ken_age(u_probs, s[:, i], settings.indv_k, True))
...
if __name__ == "__main__":
    import os
    os.environ["WANDB_API_KEY"] = "dc07f24b1e97bc82ff8e067df2c449f3407e9ced"
